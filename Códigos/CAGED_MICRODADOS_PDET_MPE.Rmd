...
DadosMensais <- fread(microdados,sep=";",header=TRUE,select= colunas)
...
DadosFiltrados<-FUNC_CLASSIFICADOR_MPE_MGE(DadosMensais)
rm(DadosMensais)  
...
FUNC_GROUPBY<-function(dados,tipo){
  #browser()
  if (tipo=="DP"){
...
acumulaFP_NPeriodos<-function(vetor_interesse=c(),mes_referencia="",consulta_padrao="ON"){
  ...
  for (i in vetor_interesse){
    ...
    for(j in vet12){
      ...
      print(nomearquivo)
      #browser()
      dados<-FUNC_LOAD_DATA(nomearquivo,"FP",extraiAno)
...
#browser()
  ...
  acumFPFull<-FUNC_GROUPBY(acumFPFull,"FP")
...
acumulaDP_NPeriodos<-function(vetor_interesse){
  ...
  for (i in vetor_interesse){
    ...
    #acumFPFull<-data.frame()
      ...
      print(nomearquivo)
      #browser()
      dados<-FUNC_LOAD_DATA(nomearquivo,"DP",extraiAno)
      #dados2<-dados %>%filter(COMPETENCIA_MOVIMENTACAO==i)
      #browser()
      ...
      dados<-data.frame(FUNC_GROUPBY(dados,"DP"))
...
FUNC_BRASIL_UF_MPE<-function(dados,val){
  ...
  segmentos<-c("Agropecuária","Administração Pública","Ensino","Serviços Méd., Odontol.",
               "Serv. Aloj. Alim. Rep. Manut","Transportes e Comunicações","Com. Adm. Imóv. Serv. Téc-prof",
               "Instituições Financeiras","Servicos","Comércio Atacadista","Comércio Varejista","Comércio",
               "Construção Civil","Serviços Industriais de Utilidade Pública","Ind. Prod. Aliment. Bebidas",
               "Ind. Calçados","Ind. Têxtil, Vestuário","Ind. Quím., Prod. Farm. Veter","Ind. Borracha, Fumo, Couros",
               "Ind. Papel, Papelão, Editor","Ind. Madeira e Mobiliários","Ind. Materiais de Transporte",
               "Ind. Materiais Elétricos e Comunicações","Ind. Mecânica","Ind. Metalúrgica","Ind. Prod. Min. Não Metálicos",
               "Indústria de Transformação","Extrativa"          )
  segmentos_codigos<-list(25,24,23,22,21,20,19,18,servicos,17,16,comercio,
                          15,14,13,12,11,10,9,8,7,6,5,4,3,2,industria,extrativa)
  ...
  # O primeiro consolidado será para o BRASIL
   if (val >=100){
    #----BRASIL----
     if (val==100 ){
      for(i in seq(1,28)){
        ValorNaFonte<-unlist(segmentos_codigos[i])
        SaldoDP_MPE <-dados %>% filter(CLASS_SEBRAE=="MPE",IBGE_SUBSETOR %in% ValorNaFonte) %>% summarize(Saldo=sum(SALDO_MOV))
        SaldoDP_MGE <-dados %>% filter(CLASS_SEBRAE=="MGE", IBGE_SUBSETOR %in% ValorNaFonte) %>% summarize(Saldo=sum(SALDO_MOV))
    ...
    #----REGIÃO NORTE----
      ...
      for(i in seq(1,28)){
        ValorNaFonte<-unlist(segmentos_codigos[i])
        SaldoDP_MPE <-dados %>% filter(CLASS_SEBRAE=="MPE",IBGE_SUBSETOR %in% ValorNaFonte, UF %in% vetor) %>% summarize(Saldo=sum(SALDO_MOV))
        SaldoDP_MGE <-dados %>% filter(CLASS_SEBRAE=="MGE", IBGE_SUBSETOR %in% ValorNaFonte, UF %in% vetor) %>% summarize(Saldo=sum(SALDO_MOV))
        DF<-data.frame("MGE"=SaldoDP_MGE$Saldo,"MPE"=SaldoDP_MPE$Saldo,"SALDO"=SaldoDP_MPE$Saldo+SaldoDP_MGE$Saldo,"MGEACUM"=0,"MPEACUM"=0,"SALDOACUM"=0)
        row.names(DF) <- segmentos[i]
        AcumResultados<-rbind(DF,AcumResultados)
      }
    ...
    for(i in seq(1,28)){
      ValorNaFonte<-unlist(segmentos_codigos[i])
      SaldoDP_MPE <-dados %>% filter(CLASS_SEBRAE=="MPE",IBGE_SUBSETOR %in% ValorNaFonte,UF %in% val) %>% summarize(Saldo=sum(SALDO_MOV))
      SaldoDP_MGE <-dados %>% filter(CLASS_SEBRAE=="MGE", IBGE_SUBSETOR %in% ValorNaFonte,UF %in% val) %>% summarize(Saldo=sum(SALDO_MOV))
      DF<-data.frame("MGE"=SaldoDP_MGE$Saldo,"MPE"=SaldoDP_MPE$Saldo,"SALDO"=SaldoDP_MPE$Saldo+SaldoDP_MGE$Saldo,"MGEACUM"=0,"MPEACUM"=0,"SALDOACUM"=0)
      row.names(DF) <- segmentos[i]
      AcumResultados<-rbind(DF,AcumResultados)
    }
...
FUNC_PLANILHA.UNICA<-function(mesreferencial,anobase){
#browser()
# verdadeiro<-  paste("0",seq(1:mesreferencial),sep="")
# falso<-c(paste("0",seq(1:9),sep=""),seq(from=10,to=mesreferencial))
# MESES<-ifelse(nchar(mesreferencial)==1,c(verdadeiro),c(falso))

a <- seq(from=1,to=mesreferencial,by=1)
MESES<-sprintf("%02d", a)

nomearquivo_doMes<-ifelse(nchar(mesreferencial)==1,paste("CAGEDEST_","0",length(MESES),anobase,sep=""),paste("CAGEDEST_",length(MESES),anobase,sep=""))

print(nomearquivo_doMes)
#browser()
...
microDados_doMes<-FUNC_LOAD_DATA(nomearquivo_doMes,"DP",anobase)
...
temp<-length(ENTIDADES)
SEQ  <- seq(1,temp)
pb   <- txtProgressBar(1, temp, style=3)
TIME <- Sys.time()
...
for (unidades in ENTIDADES){
  ...
  dados<-FUNC_LOAD_DATA(nomearquivo,tipo,anobase)
  #browser()
  #dadosDP_agrupados<-datasetAgrupadoDP(dados)
  dadosDP<-FUNC_BRASIL_UF_MPE(dados,unidades)
  rm(dados)
  ...
  dfMes<-FUNC_BRASIL_UF_MPE(microDados_doMes,unidades)

  dffinal<-cbind(dfMes[,c(1,2,3)],dfAcum[,c(1,2,3)])
  rm(dfMes)
  
Sys.sleep(0.02)
setTxtProgressBar(pb,match(unidades,ENTIDADES))
  
nome<-paste(unidades)
#arquivo<-paste("Compilado_","0",length(MESES),anobase,".xlsx",sep="")

arquivo<-paste("Compilado",".xlsx",sep="")
...
nome<-NOME_ENTIDADES[match(unidades,ENTIDADES)]
 ...
 if(unidades ==100){
  pastaDestino<-file.path("c:", "BigData", "MicrodadosCaged", "Resultados")
  setwd(pastaDestino)
  print(paste("Arquivo serão gerados em:",getwd()))
...
# Descrição: A função acumulaFP busca agrupar os dados fora do prazo(FP) desde o início do anobase de referência. Por exemplo, se o mês é Julho(07) e o anobase é 2019, a função irá acumular os dados desde janeiro(01) de 2019.
  ...
  for(i in mesesAnteriores){
    ...
    for(j in seq(i+1,mes)){
      ...
      # val<-paste(anobase,"0",i,sep="")
      #print(val)
      # nomearquivo<-ifelse(nchar(mes)==1,paste("CAGEDEST_AJUSTES_","0",length(MESES),anobase,sep=""),paste("CAGEDEST_AJUSTES_",length(MESES),anobase,sep=""))
      # 
      
      nomearquivo<-sprintf("CAGEDEST_AJUSTES_%02d", j)
      nomearquivo<-paste(nomearquivo,anobase,sep="")
      ...
      #head(dados)
      dadosFP<-FUNC_BRASIL_UF_MPE(dados,unidades)
      #head(dadosFP)
...
acumulaFP12<-function(mes,anobase){
  ...
  for(i in vetorvertical){
    ...
    for(j in vetorhorizontal){
      ...
      # }
      # 
       dados<-FUNC_LOAD_DATA(nomearquivo,"FP",anofoco)
      # #print("Leitura")
      # #head(dados)
      ...
      #print(nomearquivo)
       ...
       dadosFPAgrupados<-FUNC_GROUPBY(dados2,"FP")
      
      # #head(dados)
      # dadosFP<-FUNC_BRASIL_UF_MPE(dados,unidades)
      # #head(dadosFP)
      #browser()
      dados_tabulados<-data.frame(FUNC_CLASSIFICADOR_MPE_MGE(dadosFPAgrupados))
...
FUNC_PLANILHA.UNICA(mesreferencia,anobase)
...
nomearquivo_doMes<-ifelse(nchar(mes)==1,paste("CAGEDEST_","0",length(MESES),anobase,sep=""),paste("CAGEDEST_",length(MESES),anobase,sep=""))
...
microDados_doMes<-FUNC_LOAD_DATA(nomearquivo_doMes,"DP",anobase)
 ...
 EMPREGO_UF_MPE<-microDados_doMes %>% select(SALDO_MOV,CLASS_SEBRAE,UF) %>% group_by(UF,CLASS_SEBRAE)%>% filter(CLASS_SEBRAE=="MPE") %>% summarize(SALDO_EMPREGO=sum(SALDO_MOV))%>% arrange(desc(SALDO_EMPREGO))



EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==11] <-  "Rondônia"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==12] <-  "Acre"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==13] <-  "Amazonas"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==14] <-  "Roraima"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==15] <-  "Pará"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==16] <-  "Amapá"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==17] <-  "Tocantins"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==21] <-  "Maranhão"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==22] <-  "Piauí"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==23] <-  "Ceará"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==24] <-  "Rio Grande do Norte"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==25] <-  "Paraíba"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==26] <-  "Pernambuco"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==27] <-  "Alagoas"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==28] <-  "Sergipe"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==29] <-  "Bahia"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==31] <-  "Minas Gerais"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==32] <-  "Espírito Santo"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==33] <-  "Rio de Janeiro"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==35] <-  "São Paulo"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==41] <-  "Paraná"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==42] <-  "Santa Catarina"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==43] <-  "Rio Grande do Sul"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==50] <-  "Mato Grosso do Sul"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==51] <-  "Mato Grosso"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==52] <-  "Goiás"
EMPREGO_UF_MPE$UF[EMPREGO_UF_MPE$UF ==53] <-  "Distrito Federal"
...
EMPREGO_SETOR_MPE<-microDados_doMes %>% mutate(SETOR_ECONOMICO =if_else(IBGE_SUBSETOR %in% SIUP, "SIUP",if_else(IBGE_SUBSETOR %in% industria,"Indústria",if_else(IBGE_SUBSETOR %in% extrativa,"Extrativa",if_else( IBGE_SUBSETOR %in% construcao,"Construção",if_else(IBGE_SUBSETOR %in% comercio,"Comércio",if_else(IBGE_SUBSETOR %in% servicos,"Serviços",if_else(IBGE_SUBSETOR %in% agropecuaria,"Agropecuaria",if_else(IBGE_SUBSETOR %in% adm,"Administração Pública","OUTRO"))))))))) %>% select(SALDO_MOV,CLASS_SEBRAE,SETOR_ECONOMICO) %>% group_by(SETOR_ECONOMICO)%>% filter(CLASS_SEBRAE=="MPE") %>% summarize(SALDO_EMPREGO=sum(SALDO_MOV))%>% arrange(desc(SALDO_EMPREGO))


EMPREGO_SETOR_MPE_MGE<-microDados_doMes %>% mutate(SETOR_ECONOMICO =if_else(IBGE_SUBSETOR %in% SIUP, "SIUP",if_else(IBGE_SUBSETOR %in% industria,"Indústria",if_else(IBGE_SUBSETOR %in% extrativa,"Extrativa",if_else( IBGE_SUBSETOR %in% construcao,"Construção",if_else(IBGE_SUBSETOR %in% comercio,"Comércio",if_else(IBGE_SUBSETOR %in% servicos,"Serviços",if_else(IBGE_SUBSETOR %in% agropecuaria,"Agropecuaria",if_else(IBGE_SUBSETOR %in% adm,"Administração Pública","OUTRO"))))))))) %>% select(SALDO_MOV,CLASS_SEBRAE,SETOR_ECONOMICO) %>% mutate(SETOR_ECONOMICO=as.factor(SETOR_ECONOMICO),CLASS_SEBRAE=as.factor(CLASS_SEBRAE))%>% group_by(SETOR_ECONOMICO,CLASS_SEBRAE)%>% summarize(SALDO_EMPREGO=sum(SALDO_MOV))%>% arrange(desc(SALDO_EMPREGO))
...
# 
# #### Tratando dados FP ###########
# DadosFiltrados<-FUNC_CLASSIFICADOR_MPE_MGE(DadosMensais_FP)
# 
